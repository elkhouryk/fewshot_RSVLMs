...
def load_clip_to_cpu(cfg):
    ...
    model_path = clip._download(url, extract=(backbone_name.startswith("SkyCLIP")))

    if backbone_name == "SkyCLIP_ViT-B/32":
        # Construct the checkpoint path for SkyCLIP_ViT-B/32
        ...
        if "state_dict" in checkpoint:
            ...
            raise RuntimeError(f"SkyCLIP checkpoint does not contain a valid 'state_dict'. Keys: {list(checkpoint.keys())}")

        # Clean up the state_dict keys
        state_dict = {k.replace("module.", "").replace("model.", ""): v for k, v in state_dict.items()}

        # Build the model and load the state_dict
        model = clip.build_model(state_dict)
        model.load_state_dict(state_dict, strict=False)

    elif backbone_name == "SkyCLIP_ViT-L/14":
        # Construct the checkpoint path for SkyCLIP_ViT-L/14
        ...
        # Logic for non-SkyCLIP models
        try:
            # Attempt to load as JIT archive
            model = torch.jit.load(model_path, map_location="cpu").eval()
            state_dict = None
        except RuntimeError:
            # Fallback to loading as state_dict
            state_dict = torch.load(model_path, map_location="cpu")

        # Build the model and load the state_dict (if applicable)
